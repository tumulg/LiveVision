...
class ScreenCaptureAgent:
    def __init__(self) -> None:
        ...
        self.top_left = (1070, 1039)
        self.bottom_right = (1190, 1052)
        ...
        with mss.mss() as sct:
            while True: 
                ...
                if elapsed_time >= fps_report_delay:
                    ...
                    print("health percentage: "+ str(hue_match_pct(self.img_health_HSV, 160, 170)))
                    n_frames = 0
                    fps_report_time = time.time()
                n_frames += 1
                

def convert_hue(hue): #gimp
    ratio = 361 / 180
    return np.round(hue / ratio, 2)

def hue_match_pct(img, hue_low, hue_high): #range of hues, percentage
    match_pixels = 0
    no_match_pixels = 0
    for pixel in img:
        for h, s, v in pixel:
